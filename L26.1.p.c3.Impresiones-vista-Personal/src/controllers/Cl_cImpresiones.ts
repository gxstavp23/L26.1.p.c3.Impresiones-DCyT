...
export default class Cl_cImpresiones {
  ...
  if (this.vista.hashCodigo && this.vista.hashCodigo.trim() !== "") {
    ...
    solicitudesFiltradas = solicitudesFiltradas.filter(reg => 
      reg.modelo.codigoOrden.toLowerCase().includes(busqueda)
    );
  }
    ...
    this.vista.mostrarEstadisticas(
      ...
      this.modelo.calcularTotalIngresosUsd(),
      this.modelo.calcularPorcentajeIngresosImpresiones(),
      this.modelo.calcularPorcentajeIngresosArticulos(),
      this.modelo.calcularPorcentajeIngresosdescargas()
